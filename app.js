...
const { all, get, ready, run } = require('./database');
...
app.get('/api/users', asyncRoute(async (req, res) => {
  res.json(await all('SELECT id,name,email,avatar,bio,location,created_at FROM users'));
}));

app.get('/api/users/:id', asyncRoute(async (req, res) => {
  const user = await get('SELECT id,name,email,avatar,bio,location,created_at FROM users WHERE id=?', req.params.id);
  if (!user) return res.status(404).json({ error: 'Not found' });

  const { avg, count } = await get('SELECT AVG(rating) as avg, COUNT(*) as count FROM reviews WHERE reviewed_id=?', req.params.id);
  const listings = await all('SELECT * FROM listings WHERE user_id=? ORDER BY created_at DESC', req.params.id);
...
app.get('/api/listings/:id', asyncRoute(async (req, res) => {
  const listing = await get(`SELECT l.*,u.name as user_name,u.avatar as user_avatar
...
app.post('/api/listings', asyncRoute(async (req, res) => {
  ...
  const listing = await get('SELECT * FROM listings WHERE id=?', result.lastInsertRowid);

  const oppSide = side === 'left' ? 'right' : side === 'right' ? 'left' : null;
  let potentialMatches = [];
  if (oppSide) {
    potentialMatches = await all(`
...
app.patch('/api/listings/:id', asyncRoute(async (req, res) => {
  await run('UPDATE listings SET status=? WHERE id=?', req.body.status, req.params.id);
  res.json(await get('SELECT * FROM listings WHERE id=?', req.params.id));
}));

// ── MATCH SUGGESTIONS ────────────────────────────────────────────────────────
app.get('/api/listings/:id/suggestions', asyncRoute(async (req, res) => {
...
app.post('/api/matches', asyncRoute(async (req, res) => {
  ...
  if (String(listing_id_1) === String(listing_id_2)) {
    return res.status(400).json({ error: 'A match requires two different listings' });
  }

  const listing1 = await get('SELECT * FROM listings WHERE id=?', listing_id_1);
  const listing2 = await get('SELECT * FROM listings WHERE id=?', listing_id_2);
  if (!listing1 || !listing2) return res.status(404).json({ error: 'Listing not found' });
  if (listing1.user_id === listing2.user_id) {
    return res.status(400).json({ error: 'A match requires two different users' });
  }

  const existing = await get(`SELECT * FROM matches WHERE
    (listing_id_1=? AND listing_id_2=?) OR (listing_id_1=? AND listing_id_2=?)`,
    listing_id_1, listing_id_2, listing_id_2, listing_id_1
  );
  if (existing) return res.json(existing);
  const result = await run(`INSERT INTO matches (listing_id_1,listing_id_2,user_id_1,user_id_2,status) VALUES (?,?,?,?,'pending')`,
    listing_id_1, listing_id_2, user_id_1, user_id_2
  );
  res.json(await get('SELECT * FROM matches WHERE id=?', result.lastInsertRowid));
}));

app.patch('/api/matches/:id', asyncRoute(async (req, res) => {
  await run('UPDATE matches SET status=? WHERE id=?', req.body.status, req.params.id);
  if (req.body.status === 'completed') {
    const m = await get('SELECT * FROM matches WHERE id=?', req.params.id);
    await run("UPDATE listings SET status='matched' WHERE id=? OR id=?", m.listing_id_1, m.listing_id_2);
  }
  res.json(await get('SELECT * FROM matches WHERE id=?', req.params.id));
}));

// ── MESSAGES ─────────────────────────────────────────────────────────────────
app.get('/api/messages/:matchId', asyncRoute(async (req, res) => {
  res.json(await all(`
...
app.post('/api/messages/:matchId', asyncRoute(async (req, res) => {
  const { sender_id, content } = req.body;
  const result = await run('INSERT INTO messages (match_id,sender_id,content) VALUES (?,?,?)',
    req.params.matchId, sender_id, content
  );
  const match = await get('SELECT * FROM matches WHERE id=?', req.params.matchId);
  if (match.status === 'pending') {
    await run("UPDATE matches SET status='accepted' WHERE id=?", req.params.matchId);
  }
  res.json(await get(`SELECT m.*,u.name as sender_name,u.avatar as sender_avatar
    FROM messages m JOIN users u ON m.sender_id=u.id WHERE m.id=?`, result.lastInsertRowid
  ));
}));

// ── REVIEWS ───────────────────────────────────────────────────────────────────
app.get('/api/reviews/check', asyncRoute(async (req, res) => {
  const { reviewer_id, match_id } = req.query;
  const review = await get('SELECT id FROM reviews WHERE reviewer_id=? AND match_id=?', reviewer_id, match_id);
...
app.post('/api/reviews', asyncRoute(async (req, res) => {
  ...
  const result = await run('INSERT INTO reviews (reviewer_id,reviewed_id,match_id,rating,comment) VALUES (?,?,?,?,?)',
